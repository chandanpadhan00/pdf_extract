...
def clean_docx(docx_path, cleaned_docx_path):
    ...
    new_doc = Document()
    copy = False
    new_para = None  # Track paragraph for table insertion
    ...
    for para in doc.paragraphs:
        ...
        if section_17_pattern.search(para.text):
            copy = False
        if copy:
            if new_para is not None:  # Check if a table needs to be inserted
                new_doc.insert_paragraph(new_para, text=para.text)
                new_para = None  # Reset for next table

        for table in doc.tables:
            if new_para is None:  # If no table position tracked yet
                new_para = para  # Set current paragraph for next table

            new_table = new_doc.add_table(rows=0, cols=len(table.columns))
            new_table.style = table.style  # Copy table style
            ...
            new_doc.add_paragraph()  # Add a blank paragraph after the table
